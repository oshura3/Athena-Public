...
def sync_file_to_supabase(file_path: Path, table_name: str, extra_metadata: Dict = None, manifest: Dict = None, save_on_success: bool = True):
    """
    ...
    if table_name == "sessions":
        ...
        data["session_number"] = int(re.search(r'session-(\d+)', file_path.name).group(1)) if "session-" in file_path.name else 1
...
def sync_directory(directory: Path, table_name: str, recursive: bool = True):
    ...
    with ThreadPoolExecutor(max_workers=10) as executor:
        ...
        future_to_file = {
            executor.submit(sync_file_to_supabase, f, table_name, manifest=manifest, save_on_success=False): f 
            for f in files
        }
