-- ==============================================================================
-- MIGRATION 012: FIX ARCHITECTURE (NO INDEX + KEEP JSONB)
-- ==============================================================================
-- Purpose:
-- 1. Recover from Error 54000 (Index dimension limit < 3072).
-- 2. Ensure `metadata` JSONB column exists (safe re-run).
-- 3. DROP failing indexes: For <10k rows, exact search (sequential scan) is FAST and accurate.
-- 4. Apply the Search Function updates (to return metadata).
--
-- Why NO INDEX?
-- Your `pgvector` version is older and limits indexes to 2000 dimensions.
-- `text-embedding-3-large` uses 3072 dimensions.
-- For standard datasets (under 100k rows), exact nearest neighbor search is perfectly performant.
-- We prioritize correctness (3072 dims) over premature optimization (broken indexes).
-- ==============================================================================
-- 0. ATTEMPT UPDATE (Optional - may fail if permissions lack)
-- ==============================================================================
...
ADD COLUMN IF NOT EXISTS metadata JSONB DEFAULT '{}'::jsonb;
-- 2. DROP ALL VECTOR INDEXES (Clean Slate)
-- ==============================================================================
-- We drop both HNSW and IVFFlat attempts. We will run index-less.
-- Sessions
